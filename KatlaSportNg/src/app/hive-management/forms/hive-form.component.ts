...
export class HiveFormComponent implements OnInit {
  ...
  onDelete() {
    this.hiveService.setHiveStatus(this.hive.id, true).subscribe(() => this.hive.isDeleted = true);
  }

  onUndelete() {
    this.hiveService.setHiveStatus(this.hive.id, false).subscribe(() => this.hive.isDeleted = false);
  }
